...
export default function ReindexPage() {
  ...
  async function start() {
    ...
    try {
      // 1. Get the file list / total.
      const listRes = await fetch(
        `/api/admin/reindex?key=${encodeURIComponent(secret)}`
      );
      if (!listRes.ok) {
        const j = await listRes.json().catch(() => ({}));
        throw new Error(
          listRes.status === 401
            ? "密钥不对 (unauthorized) — 检查 ADMIN_REINDEX_SECRET"
      ...
      for (let i = 0; i < t; i++) {
        ...
        if (!res.ok) {
          setResults((r) => [
            ...r,
            { file: j.file || `file #${i + 1}`, error: j.error || `HTTP ${res.status}` },
          ]);
    ...
    <div className="mx-auto max-w-2xl px-4 py-10 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Rebuild RAG knowledge base — 把 training-data 里的税务资料重新嵌入并写入 Supabase。
          安全加固后需要跑一次,之后改了资料也可以再跑。
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <Label htmlFor="admin-secret">ADMIN_REINDEX_SECRET</Label>
          <Input
            id="admin-secret"
            type="password"
            placeholder="贴上你在 Vercel 设的密钥"
            value={secret}
            onChange={(e) => setSecret(e.target.value)}
            disabled={running}
          />
          <Button onClick={start} disabled={running || !secret}>
            ...
            <ul className="mt-2 space-y-1">
              {results.map((r, i) => (
                <li key={i} className="flex justify-between gap-3">
                  ...
                  <span
                    className={
                      r.error ? "text-red-600 shrink-0" : "text-green-600 shrink-0"
                    }
                  >
      ...
      {done && !error && (
        <div className="rounded-md border border-green-200 bg-green-50 dark:bg-green-950 dark:border-green-800 p-4 text-sm text-green-800 dark:text-green-200">
          ...
          <p>
            知识库已重建。去 /ai-tax 问一个税务问题验证 AI 能正常引用资料。
          </p>
        </div>
      )}
    </div>
